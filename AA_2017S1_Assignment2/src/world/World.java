...
/**
 * Class modelling the game "world" for a player. It includes the grid, location
 * of their ships, and all guesses from their opponents.
 * 
...
public class World
{
	...
	{
		if (isHex)
			return cdn.row >= 0 && cdn.row < numRow
					&& cdn.column >= (cdn.row + 1) / 2
					&& cdn.column < numColumn + (cdn.row + 1) / 2;
		else
			return cdn.row >= 0 && cdn.row < numRow && cdn.column >= 0
					&& cdn.column < numColumn;
	}
	 ...
	 * @param locFile
	 *            ship location input file name.
	 * @param isVisual
	 *            whether to visualise the game.
	 * 
	 * @return true if all input files are existing and valid.
	 * 
	 * @throws FileNotFoundException
	 *             when input files missing, throws exception to caller.
	 */
	public boolean setupWorld(String configFile, String locFile, int player,
			boolean isVisual) throws FileNotFoundException
	{
		this.player = player;
		...
		if (isHex)
		{
			...
			for (int r = 0; r < numRow; r++)
			{
				for (int c = d; c < d + numColumn; c++)
				{
					...
					StdDraw.line(c + 1 + shift, r + 0.5 - halfEdge, c + 1
							+ shift, r + 0.5 + halfEdge);
					StdDraw.line(c + 0.5 + shift, r + 0.5 + 2 * halfEdge, c + 1
							+ shift, r + 0.5 + halfEdge);
					StdDraw.line(c + 0.5 + shift, r + 0.5 + 2 * halfEdge, c
							+ shift, r + 0.5 + halfEdge);
					StdDraw.line(c + shift, r + 0.5 - halfEdge, c + shift, r
							+ 0.5 + halfEdge);
					StdDraw.line(c + shift, r + 0.5 - halfEdge,
							c + 0.5 + shift, r + 0.5 - 2 * halfEdge);
					StdDraw.line(c + 1 + shift, r + 0.5 - halfEdge, c + 0.5
							+ shift, r + 0.5 - 2 * halfEdge);
				}
			}
			...
			for (ShipLocation shipLoc : shipLocations)
			{
				for (Coordinate cdn : shipLoc.coordinates)
				{
					...
					double x[] = { c + 1 + shift, c + 1 + shift,
							c + 0.5 + shift, c + shift, c + shift,
							c + 0.5 + shift };
					double y[] = { r + 0.5 - halfEdge, r + 0.5 + halfEdge,
							r + 0.5 + 2 * halfEdge, r + 0.5 + halfEdge,
							r + 0.5 - halfEdge, r + 0.5 - 2 * halfEdge };
